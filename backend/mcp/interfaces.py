...
class PatchOperation(BaseModel):
    ...
    value: Optional[Any] = Field(None, description="新值（用于'set', 'add', 'replace'）")
    items: Optional[List[Any]] = Field(None, description="要添加的项目（用于'add'）")
    condition: Optional[str] = Field(None, description="可选条件")
...
class PatchInput(BaseModel):
    ...
    instance_id: str = Field(..., description="目标实例（如 'demo', 'counter', 'form'）")
                                 # 使用 "__CREATE__" 创建新实例
                                 # 使用 "__DELETE__" 删除实例
    new_instance_id: Optional[str] = Field(None, description="当 instance_id == '__CREATE__' 时必需")
    target_instance_id: Optional[str] = Field(None, description="当 instance_id == '__DELETE__' 时必需")
...
class PatchInputWithShortcuts(BaseModel):
    ...
    field_key: Optional[str] = Field(None, description="要更新/删除的字段键（用于快捷操作）")
    updates: Optional[Dict[str, Any]] = Field(None, description="字段属性字典（用于更新快捷方式）")
    remove_field: Optional[bool] = Field(False, description="如果为True，删除指定字段（用于删除快捷方式）")
    block_index: Optional[int] = Field(0, description="表单块索引（默认为0，第一个块）")
...
class CompletionCriterion(BaseModel):
    ...
    path: Optional[str] = Field(None, description="字段路径（用于字段相关标准）")
    value: Optional[Any] = Field(None, description="期望值（用于 field_value 标准）")
    count: Optional[int] = Field(None, description="期望计数（用于 block_count 标准）")
    condition: Optional[str] = Field(None, description="自定义验证表达式（用于自定义标准）")

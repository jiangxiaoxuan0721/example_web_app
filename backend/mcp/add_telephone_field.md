...
如果使用新版本的 MCP 工具，可以使用更简洁的字段快捷方式：
...
注意：对于添加字段，目前仍需要使用标准的 `add` 操作，因为字段快捷方式主要用于更新和删除操作。

**自动刷新功能**：
- 当更新或删除字段时，系统会自动刷新实例，确保前端立即显示最新状态
- 返回结果会包含 `auto_refreshed: true`，表示实例已自动刷新
- 如果刷新失败，会包含 `auto_refresh_error` 字段提供错误信息
...
这种架构改进带来以下好处：
- 更清晰的代码组织，更易于维护
- 更好的可测试性，可以独立测试工具的定义和实现
- 更好的可扩展性，可以轻松添加新工具或修改现有工具的实现
